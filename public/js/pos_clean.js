...
async function refreshCartFromServer() {
  try {
    const r = await apiFetch("/api/cart", { cache: "no-store" });
    if (!r.ok) return;
...
try {
  window.savePosCustomerPayment = savePosCustomerPayment;
} catch {}
...
// Global API helper that includes credentials by default
...
async function fetchWithCache(url, options = {}) {
  ...
  if (cachedData && now - cachedData.timestamp < API_CACHE_DURATION) {
    return new Response(
      new Blob([JSON.stringify(cachedData.data)], { type: "application/json" }),
      {
        status: 200,
        statusText: "OK",
...
// Custom price state
let customPriceModal = null;
let currentCustomPriceIndex = -1;
// Shared cart sync state
let cartSaveTimer = null;
let lastServerCartAt = 0;
let lastLocalCartAt = 0;
let suppressCartSave = false;

// --- Shared Cart Sync Helpers ---
async function saveCartToServer() {
  try {
    if (!Array.isArray(cart)) return;
    const payload = {
      items: cart,
      updatedAt: Number(lastLocalCartAt || Date.now()),
    };
    ...
    if (res.status === 409) {
      // Conflict: server has newer cart. Load it and render to avoid reappearing items.
      const j = await res.json().catch(() => ({ items: [], updatedAt: 0 }));
      const serverTs = Number(j.updatedAt || 0);
      cart = Array.isArray(j.items) ? j.items : [];
      lastLocalCartAt = serverTs;
      lastServerCartAt = serverTs;
      try {
        localStorage.setItem("pos_cart", JSON.stringify(cart));
    ...
    if (out && out.updatedAt) {
      lastServerCartAt = Number(out.updatedAt);
    } else {
      lastServerCartAt = Date.now();
    }
  } catch (e) {}
}
...
async function syncServerCartInitial() {
  try {
    const res = await apiFetch("/api/cart", { cache: "no-store" });
    if (!res.ok) return;
    const data = await res.json().catch(() => ({ items: [], updatedAt: 0 }));
    const serverTs = Number(data.updatedAt || 0);
    lastServerCartAt = serverTs;
    const localTs = Number(lastLocalCartAt || 0);
    if (serverTs > localTs) {
      cart = Array.isArray(data.items) ? data.items : [];
      try {
        localStorage.setItem("pos_cart", JSON.stringify(cart));
...
// Auto-reload cart when server (cart.json) changes
...
function clearCartEventListeners() {
...
// DOM Elements - akan diinisialisasi saat DOM siap
let productList;
let cartItems;
let cartTotal;
let checkoutBtn;
...
function initializeDOMElements() {
    ...
    shiftStatusLabel = document.getElementById("shiftStatusLabel");
    ...
    cartDiscountSpan = document.getElementById("cartDiscount");
    cartTaxSpan = document.getElementById("cartTax");
    cartServiceSpan = document.getElementById("cartService");
    resetCategoryBtn = document.getElementById("resetCategoryBtn");
    
    // Filter buttons
    filterAllBtn = document.getElementById("filterAll");
    filterTopBtn = document.getElementById("filterTop");
    filterBestBtn = document.getElementById("filterBest");
    filterDiscountedBtn = document.getElementById("filterDiscounted");
    
    // Custom Price Modal Elements
    customPriceProductNameInput = document.getElementById("customPriceProductName");
    customPriceOriginalInput = document.getElementById("customPriceOriginal");
    customPriceValueInput = document.getElementById("customPriceValue");
    confirmCustomPriceBtn = document.getElementById("confirmCustomPriceBtn");
    ...
    // POS Settings
    posSettingsBtn = document.getElementById("posSettingsBtn");
    savePosSettingsBtn = document.getElementById("savePosSettingsBtn");
    posPaperWidth = document.getElementById("posPaperWidth");
    posStoreName = document.getElementById("posStoreName");
    posShowAddress = document.getElementById("posShowAddress");
    posShowPhone = document.getElementById("posShowPhone");
    posShowFooter = document.getElementById("posShowFooter");
    modalTotal = document.getElementById("modalTotal");
    amountReceivedInput = document.getElementById("amountReceived");
    transferTotalBtn = document.getElementById("transferTotalBtn");
    ...
    qrisPaymentSection = document.getElementById("qrisPaymentSection");
    voidTransactionBtn = document.getElementById("voidTransactionBtn");
    printReceiptBtn = document.getElementById("printReceiptBtn");
    printReceiptFromDetailsBtn = document.getElementById("printReceiptFromDetailsBtn");
    
    // Variant modal elements
    variantOptionsBox = document.getElementById("variantOptions");
    confirmVariantBtn = document.getElementById("confirmVariantBtn");
...
// Dedup for any scan source (USB/camera) to avoid double add
let lastHandledScan = { text: "", time: 0 };
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn("Reload camera failed", e2);
        camReloading = false;
        return;
      }
    }
...
try {
  (async function initPosCsrf() {
    try {
      const r = await apiFetch("/api/csrf", { cache: "no-store" });
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Inisialisasi DOM elements terlebih dahulu
...
async function loadCurrentShift() {
  try {
    const res = await apiFetch("/api/shifts/current", { cache: "no-store" });
...
async function loadQrisImage() {
  try {
    const res = await apiFetch("/api/qris", { cache: "no-store" });
...
async function loadCustomers() {
  try {
    const res = await apiFetch("/api/customers", { cache: "no-store" });
    if (!res.ok) {
...
function renderProducts() {
  console.log("renderProducts called, products.length:", products.length);
  if (!productList) {
    console.warn("productList element not found!");
    return;
  }
  ...
  try {
    ...
    // Clear existing cart event listeners
    clearCartEventListeners();
    ...
    // Apply pattern border effect to product cards
    try {
      const productCards = productList.querySelectorAll('.product-card');
      productCards.forEach((card) => {
        createPatternBorderForCard(card);
      });
    } catch (e) {
      console.warn("Error applying pattern border effect:", e);
    }
...
async function loadDrafts() {
  ...
  try {
    const res = await apiFetch("/api/drafts", { cache: "no-store" });
    if (!res.ok) {
...
// Open POS customer payment modal
async function openPosCustomerPayment(debtId) {
  ...
  const transaction = recentTransactions.find((t) => t.id === debtId);
  if (!transaction) {
  ...
  if (paymentContent) {
    paymentContent.innerHTML = `
      ...
      <div class="mb-3">
        <label for="paymentAmount" class="form-label"><strong>Jumlah Pembayaran:</strong></label>
        <input type="number" class="form-control" id="paymentAmount" min="0" max="${debt.remainingAmount}" step="1000" value="${debt.remainingAmount}" required>
        <small class="text-muted">Maksimal: ${formatCurrency(debt.remainingAmount)}</small>
      </div>
      <div class="mb-3">
        <label for="paymentNote" class="form-label"><strong>Catatan (opsional):</strong></label>
...
async function savePosCustomerPayment(debtId) {
  ...
  const paymentNote = paymentNoteInput?.value || '';
...
// Show customer debt details modal on POS page
function showPosCustomerDebtDetails(debtId) {
  ...
  // Find the original transaction from recentTransactions
  const transaction = recentTransactions.find((t) => t.id === debtId);
  if (!transaction) {
    console.error('Transaction not found for debt:', debtId);
    alert('Transaksi tidak ditemukan.');
    return;
  }
  ...
  const modalContent = document.getElementById('customerDebtDetailsContent');
  if (modalContent) {
    modalContent.innerHTML = `
      ...
      <div class="table-responsive">
          <table class="table">
              ...
              <tfoot><tr><th colspan="3">Total</th><th>${formatCurrency(transaction.totalAmount || 0)}</th></tr></tfoot>
          </table>
      </div>
      <hr>
      <div class="row">
        <div class="col-md-4">
          ...
          <p class="fs-5">${formatCurrency(debt.totalAmount)}</p>
        </div>
        <div class="col-md-4">
          <p><strong>Sudah Dibayar:</strong></p>
          <p class="fs-5">${formatCurrency(debt.paidAmount)}</p>
        </div>
        <div class="col-md-4">
          <p><strong>Sisa Hutang:</strong></p>
          <p class="fs-5 text-danger">${formatCurrency(debt.remainingAmount)}</p>
        </div>
      </div>
    `;
  }
  ...
  if (payBtn) {
    payBtn.onclick = () => {
      try {
        const modal = bootstrap.Modal.getInstance(document.getElementById('customerDebtDetailsModal'));
        if (modal) modal.hide();
...
function renderRecentTransactions(transactions) {
  ...
  if (transactions.length === 0) {
    ...
    // Update customer debts even if no transactions
    updatePosCustomerDebts(transactions);
    return;
  }
...
function showTransactionDetails(transactionId) {
  ...
  if (transactionDetailsContent) {
    ...
    const receiptHTML =
      "<!DOCTYPE html><html><head><title>Receipt</title><style>@page { size: " +
      widthMm +
      " auto; margin: 0; }body { font-family: 'Courier New', Courier, monospace; padding: 10px; font-size: " +
      fontSize +
      "; }.receipt { width: " +
      widthMm +
      "; margin: 0 auto; }h1 { text-align: center; }.details { margin: 20px 0; }.details p { margin: 5px 0; }table { width: 100%; border-collapse: collapse; }th, td { border: 1px dashed #000; padding: 8px; text-align: left; }th { border-bottom: 2px solid #000; }.total { border-top: 2px solid #000; font-weight: bold; }.footer { margin-top: 30px; text-align: center; font-size: " +
      footerFontSize +
      '; }@media print { body { padding: 0; } }</style></head><body><div class="receipt"><div style="text-align:center; margin-bottom: 10px;">' +
      (appSettings && appSettings.logoBase64
        ? '<img src="' +
          appSettings.logoBase64 +
          '" style="max-height:60px; object-fit:contain;" />'
        : "") +
      '<h1 style="margin: 6px 0 0 0;">' +
      (appSettings?.storeName || "STRUK PENJUALAN") +
      '</h1></div><div class="details"><p><strong>ID Transaksi:</strong> ' +
      transaction.id +
      "</p><p><strong>Tanggal:</strong> " +
      new Date(transaction.timestamp).toLocaleDateString() +
      "</p><p><strong>Pelanggan:</strong> " +
      (transaction.customerName || "Pelanggan Umum") +
      "</p>" +
      (showAddr && appSettings && appSettings.address
        ? "<p><strong>Alamat:</strong> " + appSettings.address + "</p>"
        : "") +
      (showPhone && appSettings && appSettings.phone
        ? "<p><strong>Telepon:</strong> " + appSettings.phone + "</p>"
        : "") +
      "</div><table><thead><tr><th>Item</th><th>Harga</th><th>Jml</th><th>Total</th></tr></thead><tbody>" +
      transaction.items
        .map((item) => {
          const itemName = item.name || "Item Tidak Dikenal";
          const itemPrice = item.price || 0;
      ...
      (typeof transaction.subtotal === "number"
        ? '<tr><td colspan="3">Subtotal</td><td>' +
          formatCurrency(transaction.subtotal) +
          "</td></tr>"
        : "") +
      (typeof transaction.discountAmount === "number" &&
      transaction.discountAmount > 0
        ? '<tr><td colspan="3">Diskon</td><td>- ' +
          formatCurrency(transaction.discountAmount) +
          "</td></tr>"
        : "") +
      (typeof transaction.taxAmount === "number" && transaction.taxAmount > 0
        ? '<tr><td colspan="3">Pajak</td><td>' +
          formatCurrency(transaction.taxAmount) +
          "</td></tr>"
        : "") +
      (typeof transaction.serviceAmount === "number" &&
      transaction.serviceAmount > 0
        ? '<tr><td colspan="3">Layanan</td><td>' +
          formatCurrency(transaction.serviceAmount) +
          "</td></tr>"
        : "") +
      '<tr class="total"><td colspan="3">TOTAL</td><td>' +
      formatCurrency(transaction.totalAmount || 0) +
      '</td></tr></tfoot></table><div class="details"><p><strong>Metode Pembayaran:</strong> ' +
      (transaction.paymentMethod
        ? transaction.paymentMethod.toUpperCase()
        : "UNKNOWN") +
      "</p>" +
      (transaction.paymentMethod === "cash"
        ? "<p><strong>Jumlah Diterima:</strong> " +
          formatCurrency(transaction.amountReceived || 0) +
          "</p><p><strong>Kembalian:</strong> " +
      ...
      (showFooter
        ? '<div class="footer"><p>' +
          (appSettings?.receiptFooter || "Terima kasih atas pembelian Anda!") +
          "</p>" +
          (appSettings?.receiptFooter1 && appSettings.receiptFooter1.trim()
            ? "<p>" + appSettings.receiptFooter1 + "</p>"
            : "") +
          "</div>"
        : "") +
      "</div></body></html>";
...
async function generateReceiptPDF(transaction) {
  return new Promise((resolve, reject) => {
    const run = async () => {
      ...
      const widthPx = Math.max(200, Math.round(paperWidth * 3.78));
      let fontSize, footerFontSize;
      if (paperWidth <= 58) {
        fontSize = "12px";
        footerFontSize = "0.8em";
      ...
      receipt.innerHTML =
        ...
        (transaction.customerName || "Pelanggan Umum") +
        "</p>" +
        (showAddr && appSettings?.address
          ? "<p><strong>Alamat:</strong> " + appSettings.address + "</p>"
          : "") +
        (showPhone && appSettings?.phone
          ? "<p><strong>Telepon:</strong> " + appSettings.phone + "</p>"
          : "") +
        "</div>" +
      ...
      await waitImages(receipt);

      // Primary: render to canvas, verify non-blank, then compose jsPDF
      const h2c =
        window.html2canvas ||
      ...
      // Quick whiteness check (sampled grid) to avoid blank output
      try {
        ...
        for (
          let y = 0;
          y < canvas.height;
          y += Math.max(1, Math.floor(canvas.height / sample))
        ) {
          for (
            let x = 0;
            x < canvas.width;
            x += Math.max(1, Math.floor(canvas.width / sample))
          ) {
        ...
        if (whiteRatio > 0.985) throw new Error("Canvas appears blank");
      } catch (e) {
        // If sampling fails, continue to fallback
        throw e;
      }

      const imgData = canvas.toDataURL("image/jpeg", 0.98);
      const heightPx = canvas.height;
      const heightMm = Math.max(40, Math.ceil(heightPx / 3.78));
      const jsPDFCtor =
    ...
    run().catch(async (primaryErr) => {
      // Fallback 1: html2pdf direct pipeline
      try {
        const paperWidth = parseInt(appSettings?.paperWidth) || 80;
        const widthPx = Math.max(200, Math.round(paperWidth * 3.78));
        const el = document.querySelector("div.receipt");
        const target = el || document.body;
        const opt = {
          margin: 0,
          filename: "receipt_" + (transaction?.id || Date.now()) + ".pdf",
        ...
        // Fallback 2: Pure jsPDF text rendering
        try {
          const jsPDFCtor =
            window.jspdf && window.jspdf.jsPDF
              ? window.jspdf.jsPDF
          ...
          if (appSettings?.storeName) {
            line(appSettings.storeName, 12, true);
          } else {
            line("STRUK PENJUALAN", 12, true);
          }
          line("ID: " + transaction.id, 9);
          line(
            "Tanggal: " + new Date(transaction.timestamp).toLocaleString(),
            9
          );
          line(
          ...
          if (appSettings?.showReceiptAddress !== false && appSettings?.address)
            line("Alamat: " + appSettings.address, 8);
          if (appSettings?.showReceiptPhone !== false && appSettings?.phone)
            line("Telepon: " + appSettings.phone, 8);
          y += 2;
          ...
          (transaction.items || []).forEach((it) => {
            ...
            const row =
              name +
              " " +
              formatCurrency(price).replace(/\s/g, "").padStart(8, " ") +
              " " +
              String(qty).padStart(3, " ") +
              " " +
              formatCurrency(sub).replace(/\s/g, "").padStart(8, " ");
            line(row, 8);
          });
          line("-------------------------------------------", 9);
          if (typeof transaction.subtotal === "number")
            line("Subtotal: " + formatCurrency(transaction.subtotal), 9);
          if (
            typeof transaction.discountAmount === "number" &&
            transaction.discountAmount > 0
          )
            line("Diskon: -" + formatCurrency(transaction.discountAmount), 9);
          if (
            typeof transaction.taxAmount === "number" &&
            transaction.taxAmount > 0
          )
            line("Pajak: " + formatCurrency(transaction.taxAmount), 9);
          if (
            typeof transaction.serviceAmount === "number" &&
            transaction.serviceAmount > 0
          )
            line("Layanan: " + formatCurrency(transaction.serviceAmount), 9);
          line(
          ...
          if (transaction.paymentMethod === "cash") {
            line(
              "Diterima: " + formatCurrency(transaction.amountReceived || 0),
              9
            );
          ...
          if (appSettings?.showReceiptFooter !== false) {
            ...
            if (
              appSettings?.receiptFooter1 &&
              appSettings.receiptFooter1.trim()
            )
              line(appSettings.receiptFooter1, 8);
          }
          try {
            doc.internal.pageSize.setHeight(Math.max(40, y + margin));
          } catch {}
          doc.save("receipt_" + transaction.id + ".pdf");
          resolve();
        } catch (textErr) {
          reject(primaryErr || fallbackErr || textErr);
        }
      }
    });
  });
...
async function printReceipt(transaction) {
  try {
    ...
    const html =
      ...
      " auto; margin: 0 } body { font-family: 'Courier New', Courier, monospace; padding: 10px; font-size: " +
      fontSize +
      "; margin:0; } .receipt { width: " +
      widthMm +
      "; margin: 0 auto; } h1 { text-align:center; margin:6px 0 0 0; } .details { margin:20px 0; } .details p { margin:5px 0; } table { width:100%; border-collapse:collapse; } th,td { border:1px dashed #000; padding:8px; text-align:left; } th { border-bottom:2px solid #000; } .total { border-top:2px solid #000; font-weight:bold; } .footer { margin-top:30px; text-align:center; font-size:" +
      footerFontSize +
    ...
    if (!win) {
      alert(
        "Popup diblokir oleh browser. Izinkan popup untuk situs ini agar bisa mencetak."
      );
      return;
    }
    win.document.open();
    ...
    win.document.close();
    // Tunggu sedikit agar konten ter-render, lalu panggil print
    setTimeout(() => {
      try {
        win.focus();
        win.print();
...
function resetPaymentMethodToDefault() {
  ...
  // Clear localStorage payment method
  try {
...
function setupEventListeners() {
  ...
  if (checkoutBtn) {
    checkoutBtn.addEventListener("click", async () => {
      ...
      // Reset payment method to default "Cash" when opening checkout
  ...
  // Transfer total amount to amount received button
  if (transferTotalBtn) {
...
// Function to create pattern border effect for product cards
function createPatternBorderForCard(cardElement) {
  try {
    // Create wrapper structure
    const wrapper = document.createElement('div');
    wrapper.className = 'product-card-wrapper';
    
    const patternBorder = document.createElement('div');
    patternBorder.className = 'product-card-pattern-border';
    
    const inner = document.createElement('div');
    inner.className = 'product-card-inner';
    
    // Move original card content to inner
    while (cardElement.firstChild) {
      inner.appendChild(cardElement.firstChild);
    }
    
    // Assemble the structure
    wrapper.appendChild(patternBorder);
    wrapper.appendChild(inner);
    
    cardElement.appendChild(wrapper);
    
  } catch (error) {
    console.warn('Error creating pattern border for card:', error);
  }
}
